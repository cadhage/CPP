...
int main(){
	...
	vector<int> src(cnt,1);
	vector<int> dst(cnt,1);
	for(auto& e:g.edges){
		if(c[e.from]!=c[e.to]){
			src[c[e.to]]=0;
			dst[c[e.from]]=0;
	...
	int srcs=accumulate(src.begin(),src.end(),0);
	int dsts=accumulate(dst.begin(),dst.end(),0);
	int zeros=0;
	for(int i=0;i<n;i++){
		if(a[i]==0){
			zeros++;
	...
	cout<<(cnt==1?0:max(srcs,dsts))+zeros<<"\n";
	return 0;
}
